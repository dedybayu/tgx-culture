...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <p class="text-sm text-slate-500">Kelola dan kelompokkan katalog budaya Trenggalek berdasarkan kategori
                    spesifik.</p>
            </div>
            <div>
        ...
        <div class="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                ...
                <span
                    class="bg-slate-50 border border-slate-200 text-slate-500 text-xs px-2.5 py-0.5 rounded-full font-medium">{{ $categories->total() }}
                    Item</span>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        @forelse($categories as $index => $k)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4 font-medium text-slate-500">{{ $categories->firstItem() + $index }}
                                </td>
                                <td class="px-6 py-4">
                                    <div
                                        class="w-12 h-12 rounded-lg overflow-hidden border border-slate-100 bg-slate-50 flex items-center justify-center">
                                        @php
                                            $localFileExists =
                                                $k->path_gambar && file_exists(public_path($k->path_gambar));
                                            $imgUrl = $localFileExists
                                                ? asset($k->path_gambar)
                                                : asset('assets/no-image.png');
                                        @endphp
                                        <img src="{{ $imgUrl }}" alt="{{ $k->nama_kategori }}"
                                ...
                                <td class="px-6 py-4">
                                    <code
                                        class="text-xs bg-slate-50 px-2 py-1 rounded border border-slate-100 text-slate-600">{{ $k->path_gambar }}</code>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="inline-flex gap-2 justify-center">
                                        <button onclick="openEditModal(this)"
                                            class="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
                                            title="Ubah" data-id="{{ $k->kategori_id }}"
                                            data-nama="{{ $k->nama_kategori }}" data-gambar="{{ $imgUrl }}">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </button>
                                        <button
                                            onclick="openDeleteModal('{{ route('admin.kategori.destroy', $k->kategori_id) }}')"
                                            type="button"
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div
                ...
                <form action="{{ route('admin.kategori.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Upload Gambar
                            (Opsional)</label>
                        <input type="file" name="path_gambar" accept="image/*"
                            class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-900 file:text-white hover:file:bg-slate-800">
                    </div>
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div
                ...
                <form id="editForm" action="" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Gambar Saat
                            Ini</label>
                        ...
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Ganti Gambar
                            (Opsional)</label>
                        <input type="file" name="path_gambar" accept="image/*"
                            class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-900 file:text-white hover:file:bg-slate-800">
                    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div
                ...
                <p class="text-sm text-slate-500 mb-6">Apakah Anda yakin ingin menghapus kategori ini? Semua katalog yang
                    terhubung akan kehilangan kategorinya.</p>
