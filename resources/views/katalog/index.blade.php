...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div>
            <a href="{{ route('admin.katalog.create') }}" class="inline-flex items-center gap-2 px-4.5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-sm font-bold rounded-xl shadow-sm transition-all">
    ...
    <div class="space-y-4">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="inline-flex gap-2 justify-center">
                                    ...
                                    <a href="{{ route('admin.katalog.edit', $k->katalog_id) }}" class="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors flex items-center justify-center" title="Ubah">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                    <button onclick="openDeleteModal('{{ route('admin.katalog.destroy', $k->katalog_id) }}')" type="button" class="p-2 text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Hapus">
        ...
        <div class="md:hidden space-y-4">
            <div class="bg-slate-100/50 border border-slate-200/60 rounded-xl px-4 py-2.5 flex items-center justify-between text-xs font-bold text-slate-700">
            ...
            @forelse($katalogs as $k)
                <div class="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-4">
                    <div class="space-y-1">
                        ...
                        <p class="text-xs text-slate-500">Pencipta: <span class="font-medium text-slate-700">{{ $k->pencipta }}</span></p>
                    </div>

                    <div class="grid grid-cols-2 gap-2 text-xs text-slate-600 bg-slate-50 p-3 rounded-xl">
                        <div>
                            <span class="block text-[9px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Tipe</span>
                            <span class="font-bold text-slate-700">{{ $k->tipe }}</span>
                        </div>
                        <div>
                            <span class="block text-[9px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Format</span>
                            <span class="font-bold text-slate-700">{{ $k->format }}</span>
                        </div>
                        <div class="col-span-2 mt-1 pt-1.5 border-t border-slate-200/40">
                            <span class="block text-[9px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Lokasi</span>
                    ...
                    <div class="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                        <a href="{{ route('admin.katalog.show', $k->katalog_id) }}" class="inline-flex items-center justify-center gap-1 px-3 py-2 bg-slate-50 hover:bg-slate-100 text-emerald-600 text-xs font-bold rounded-xl transition-all border border-slate-100">
                            <i class="fa-solid fa-eye text-[10px]"></i>
                            Detail
                        </a>
                        <a href="{{ route('admin.katalog.edit', $k->katalog_id) }}" class="inline-flex items-center justify-center gap-1 px-3 py-2 bg-slate-50 hover:bg-slate-100 text-indigo-600 text-xs font-bold rounded-xl transition-all border border-slate-100">
                            <i class="fa-solid fa-pen-to-square text-[10px]"></i>
                            Ubah
                        </a>
                        <button onclick="openDeleteModal('{{ route('admin.katalog.destroy', $k->katalog_id) }}')" type="button" class="inline-flex items-center justify-center gap-1 px-3 py-2 bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs font-bold rounded-xl transition-all border border-rose-100">
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="relative w-full max-w-sm transform overflow-hidden rounded-2xl bg-white p-6 shadow-2xl transition-all border border-slate-100 text-center">
            ...
            <p class="text-sm text-slate-500 mb-6">Apakah Anda yakin ingin menghapus item katalog ini? Tindakan ini tidak dapat dibatalkan.</p>
